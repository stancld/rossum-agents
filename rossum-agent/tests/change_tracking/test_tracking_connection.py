...
class TestUnwrap:
    def test_unwraps_result_key(self):
        ...
        assert _unwrap(data) == {"id": 1, "name": "Schema"}

    def test_no_result_key(self):
        data = {"id": 1, "name": "Queue"}
        assert _unwrap(data) == {"id": 1, "name": "Queue"}

    def test_result_key_non_dict(self):
        data = {"result": [1, 2, 3]}
        assert _unwrap(data) == {"result": [1, 2, 3]}
...
class TestAutoCommitOnEntityConflict:
    ...
    def committed_changes(self, conn):
        """Set up a commit callback that captures flushed changes."""
        ...
        conn._commit_callback = capture_commit
        return batches
...
class TestRuleTracking:
    ...
    async def test_create_then_delete_rule_auto_commits(self, rule_conn):
        ...
        rule_conn._commit_callback = capture
...
class TestRedisCacheHelpers:
    ...
    def test_cache_set_writes_to_both_redis_and_memory(self, redis_conn, mock_redis):
        data = {"id": 2, "name": "Schema"}
        redis_conn._cache_set("schema", "2", data)

        # In-memory fallback also populated
        assert redis_conn._read_cache[("schema", "2")] == data
