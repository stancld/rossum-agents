...
class TestCallMcpTool:
    ...
    def test_raises_runtime_error_when_mcp_connection_not_set(self):
        ...
        set_context(AgentContext(mcp_connection=None, mcp_event_loop=None))
        try:
            with pytest.raises(RuntimeError, match="MCP connection not set"):
    ...
    def test_raises_runtime_error_when_only_connection_set(self):
        ...
        set_context(AgentContext(mcp_connection=mock_connection, mcp_event_loop=None))
        try:
            with pytest.raises(RuntimeError, match="MCP connection not set"):
                call_mcp_tool("get_schema", {"schema_id": "123"})
        finally:
