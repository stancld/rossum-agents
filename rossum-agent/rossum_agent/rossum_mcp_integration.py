...
class MCPConnection:
    ...
    _commit_callback: Callable[[], None] | None = field(default=None, init=False, repr=False)
    ...
    async def _handle_write(self, name: str, arguments: dict[str, Any]) -> Any:
        ...
        if operation == "create":
            ...
            if after is not None and entity_id is None and entity_type:
                source = _unwrap(after)
                entity_id = str(source.get("id", source.get(f"{entity_type}_id", "")))
        elif operation == "delete":
            after = None
        elif entity_type and entity_id:
            after = await self._fetch_snapshot(entity_type, entity_id)

        # Update cache so next write to this entity gets correct "before"
