...
def _resolve_credentials(
    ...
    """Resolve AWS credentials into kwargs for Anthropic Bedrock clients."""
...
def create_bedrock_client(
    ...
    Supports: explicit boto3.Session, AWS profile, env vars, or IAM role credentials.
    """
    return AnthropicBedrock(**_resolve_credentials(aws_region, aws_profile, session))


def create_async_bedrock_client(
    aws_region: str | None = None, aws_profile: str | None = None, session: boto3.Session | None = None
) -> AsyncAnthropicBedrock:
    """Async version of create_bedrock_client."""
    return AsyncAnthropicBedrock(**_resolve_credentials(aws_region, aws_profile, session))
